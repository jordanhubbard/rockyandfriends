...
The Rust rewrite (`rcc-server`) is 6,809 LOC and covers the core API routes, but **nothing deploys it**. Production still runs `node rcc/api/index.mjs`. The new thing is written and tested; the old thing is still running. That's the single biggest issue.
...
### 1. `rcc-hub/` — Dead fork
11 files, own package.json, zero imports from anywhere, not running on any host. This is a stale fork of `rcc/` from before the workspace reorganization. **Delete the entire directory.**

### 2. `rcc/wasm-dashboard/` — Superseded
3 Rust files that diverged from `rcc/dashboard/dashboard-ui/`. The canonical dashboard-ui ships; this doesn't. **Delete.**
...
- `rcc/heartbeat-local/server.mjs` (238L) — the one with its own package.json
- `rcc/agent/offline-heartbeat.mjs` (94L) — compact inline version
...
- `rcc/vector/index.mjs` (680L) is the canonical Milvus client — now dead code
...
- `rcc/trust/adaptive-trust.mjs` (193L) — canonical implementation
- `rcc/guardrails/adaptive-trust.mjs` (65L) — shim that re-exports from trust/
...
- `rcc/heartbeat-local/server.mjs` (238L) — standalone server with own package.json
- `offline-heartbeat/server.mjs` (217L) — near-identical copy at top level
- `rcc/agent/offline-heartbeat.mjs` (94L) — lightweight inline version

**Pick `rcc/heartbeat-local/` as canonical. Delete the other two.** (Or pick the 94L inline version if the full server is overkill now that Hermes handles heartbeats.)

### 9. Ollama Watchdog × 4
Four copies of the same watchdog:
- `ollama-watchdog.mjs` (top-level) — original
- `rcc/scripts/ollama-watchdog.mjs` (401L) — most complete
- `rcc/agent/ollama-watchdog.mjs` (215L) — agent-embedded version
- `workqueue/scripts/ollama-health-watchdog.mjs` — workqueue copy

**Keep `rcc/scripts/ollama-watchdog.mjs` (401L, most complete). Delete the other three.** Long-term this should be a Rust binary, but one JS copy is fine for operational tooling.
...
- `rcc/api/routes/agentos.mjs` (467L)
...
### 11. rcc-server (Rust) — Written But Not Deployed
...
But production still runs `node rcc/api/index.mjs` (2,134L) via `ccc-api.service`. The Makefile, Dockerfile, docker-compose.yml, and .service files all point at Node.
...
- `rcc/clawfs-sync/` — Rust implementation (has Cargo.toml)
...
Contains only a README.md. The real dashboard is at `rcc/dashboard/` (Rust workspace). **Delete the empty directory.**
...
```
CCC/
├── rcc/              # Core platform (Rust workspace)
...
That's 8 directories instead of 24. Everything else either moves under `rcc/`, moves under `deploy/`, or gets deleted.
...
### Missing: Tests
The Rust code has no test files visible outside the source. The Node.js code has `rcc/brain/test.mjs` (429L) and that's about it for the core API. For a system managing a fleet of AI agents, the test coverage is essentially zero.
...
4. **Ship rcc-server to production** (item 11) — half day, biggest impact
