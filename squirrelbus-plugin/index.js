...
const SQUIRRELBUS_TOKEN = process.env.SQUIRRELBUS_TOKEN || 'SQUIRRELBUS_TOKEN_REMOVED';
const PLUGIN_ID = 'squirrelbus-receiver';
...
function getLocalBusLogPath(workspaceDir) {
  const dir = workspaceDir
    ? join(workspaceDir, 'squirrelbus')
    : join(process.env.HOME || '/tmp', '.openclaw', 'workspace', 'squirrelbus');
...
const definition = {
  ...
  register(api) {
    ...
    // ── HTTP route: POST /squirrelbus/receive ──────────────────────────────
    api.registerHttpRoute({
      path: '/squirrelbus/receive',
