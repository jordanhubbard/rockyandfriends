#!/bin/bash
# RCC Agent — one-liner installer for Linux
# Onboards this machine as an agent node to an existing RCC Hub.
...
if [ -z "$HUB_URL" ]; then
  HUB_URL=$(ask "RCC Hub URL (e.g. http://1.2.3.4:8789)")
fi
...
# RCC Agent config — generated by install-agent.sh
...
[Unit]
Description=RCC Agent Heartbeat
...
[Unit]
Description=RCC Agent Heartbeat (every 5 min)
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo -e "${GREEN}  ✅  Agent '$AGENT_NAME' registered with RCC Hub!${NC}"
echo "════════════════════════════════════════════════════════════"
echo ""
echo "  Config:  ~/.rcc/.env"
echo "  Hub:     $HUB_URL"
