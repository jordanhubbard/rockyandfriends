...
#   - Config reads from ~/.ccc/.env (CCC_SAFETY_LEVEL) or agent's
...
#   - Audit trail appended to ~/.ccc/logs/guard.jsonl
#
# Safety Levels:
...
# Priority: CCC_SAFETY_LEVEL env var > .ccc/.env > default (1)
SAFETY_LEVEL="${CCC_SAFETY_LEVEL:-}"
if [ -z "$SAFETY_LEVEL" ] && [ -f "$CCC_DIR/.env" ]; then
  SAFETY_LEVEL=$(grep '^CCC_SAFETY_LEVEL=' "$CCC_DIR/.env" 2>/dev/null | cut -d'=' -f2 | tr -d '"' || echo "")
fi
