...
1. `CCC_SAFETY_LEVEL` environment variable
2. `CCC_SAFETY_LEVEL=2` in `~/.ccc/.env`
...
// .claude/settings.json
{
  "hooks": {
    "preToolUse": [
      {
        "matcher": "Bash",
        "command": "~/.ccc/workspace.ccc/hooks/dangerous-command-guard.sh \"$INPUT\""
      }
    ]
  }
}
...
The hook is installed to `~/.ccc/workspace.ccc/hooks/` on every `git pull`. Each agent's HEARTBEAT or startup should verify the hook is executable.

### Audit Trail

Every guard event is logged to `~/.ccc/logs/guard.jsonl`:
```json
...
- Agent infrastructure destruction (`.ccc`, `.openclaw`, `rockyandfriends`)
