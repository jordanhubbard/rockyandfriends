...
async fn upload_file(path: &Path, key: &str, backend: &Backend, agent_name: &str) {
    ...
    match backend {
        ...
        Backend::Ccc { http, endpoint } => match String::from_utf8(bytes) {
            Ok(content) => {
                    ...
                    .await
                {
                    Ok(r) if r.status().is_success() => {
                        info!("uploaded {} → ccc:{} ({} B)", path.display(), key, len)
                    }
                    Ok(r) => error!("ccc upload {} HTTP {}", key, r.status()),
                    Err(e) => error!("ccc upload {} failed: {}", key, e),
                }
            }
...
async fn sync_pull(backend: &Backend, prefix: &str, local_dir: &Path, agent_name: &str) {
    match backend {
        ...
        Backend::Ccc { http, endpoint } => {
            sync_pull_ccc(http, endpoint, prefix, local_dir, agent_name).await
        }
    }
}
...
async fn sync_pull_ccc(
        ...
        .await
    {
        Ok(r) => match r.json().await {
            Ok(v) => v,
            Err(e) => {
                error!("ccc list parse prefix={} failed: {}", prefix, e);
                return;
            }
        },
        Err(e) => {
            error!("ccc list prefix={} failed: {}", prefix, e);
            return;
        }
    };
            ...
            .await
        {
            Ok(r) if r.status().is_success() => match r.bytes().await {
                Ok(bytes) => match tokio::fs::write(&local_path, &bytes).await {
                    Ok(_) => info!("ccc downloaded {} → {} ({} B)", key, local_path.display(), bytes.len()),
                    Err(e) => error!("write {} failed: {}", local_path.display(), e),
                },
                Err(e) => error!("ccc read body {} failed: {}", key, e),
            },
            Ok(r) => error!("ccc read {} HTTP {}", key, r.status()),
            Err(e) => error!("ccc read {} failed: {}", key, e),
        }
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ccc_endpoint = std::env::var("CCC_AGENTFS_ENDPOINT")
        .ok()
        .filter(|s| !s.is_empty());
    ...
    // ── Backend ───────────────────────────────────────────────────────────────
    let backend: Arc<Backend> = if let Some(ref url) = ccc_endpoint {
