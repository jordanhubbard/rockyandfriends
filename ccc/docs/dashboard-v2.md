...
Browser
  └─ WASM (Leptos CSR) ←─ served by
       dashboard-server :8790
           ├── GET /api/*   ──proxy─→ .ccc/api :8789  (auth injected server-side)
           ├── GET /bus/*   ──proxy─→ .ccc/api :8789
           ├── GET /bus/stream  ─SSE─→.ccc/api :8789  (streamed, no timeout)
...
|---------------------|--------------------------|--------------------------------------------|
| `CCC_DASHBOARD_PORT`| `8790`                   | Change to 8788 after cutover               |
...
Trunk outputs the WASM bundle + JS glue + CSS to .ccc/dashboard/dist/`.
...
# Run tests
TEST_PORT=8790 node.ccc/tests/dashboard/smoke.test.mjs
```
...
2. Verify smoke tests pass: `TEST_PORT=8790 node.ccc/tests/dashboard/smoke.test.mjs`
3. Do a manual review at `http://localhost:8790`
4. Stop the Node.js dashboard: `systemctl stop ccc-dashboard-legacy` (or kill process)
5. Change `CCC_DASHBOARD_PORT=8788` in the v2 systemd unit and restart
