...
**Design:** Rocky  
**UI:** Natasha  
**Target:** `/projects/:id` detail page in CCC API (.ccc/api/index.mjs`)
...
| File | Change |
|------|--------|
| .ccc/api/index.mjs` | Add `GET /api/projects/:owner/:repo/github` handler + 5-min cache |
| .ccc/api/index.mjs` | Update `projectDetailHtml()` to fetch + render GitHub section |
