 ...
 * Run: node --test.ccc/tests/api.test.mjs
 *
...
const TEST_QUEUE  = join(tmpdir(), `ccc-api-queue-${Date.now()}.json`);
const TEST_AGENTS = join(tmpdir(), `ccc-api-agents-${Date.now()}.json`);
const TEST_CAPS   = join(tmpdir(), `ccc-api-caps-${Date.now()}.json`);
const TEST_CONVS  = join(tmpdir(), `ccc-api-convs-${Date.now()}.json`);
const TEST_USERS  = join(tmpdir(), `ccc-api-users-${Date.now()}.json`);
const TEST_PROJS  = join(tmpdir(), `ccc-api-projs-${Date.now()}.json`);

// Use first token as admin (matches CCC_ADMIN_TOKEN logic)
const ADMIN_TOKEN = 'api-test-admin-token';
const AGENT_TOKEN = 'api-test-agent-token';

process.env.CCC_PORT             = String(TEST_PORT);
...
process.env.CCC_ADMIN_TOKEN      = ADMIN_TOKEN;
process.env.BRAIN_STATE_PATH     = join(tmpdir(), `ccc-api-brain-${Date.now()}.json`);
