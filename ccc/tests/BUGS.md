...
- **Expected:** .ccc/ideation/ideation.mjs` exports `generateIdea` function, used by POST /api/ideation/generate
...
- **Status:** open — worked around by creating a stub at .ccc/ideation/ideation.mjs` for test purposes
...
- **Route:** N/A (import error)
- **Expected:** .ccc/vector/index.mjs` exports: `upsert`, `search`, `searchAll`, `collectionStats`
...
- **Status:** open — worked around by adding compatibility alias exports to .ccc/vector/index.mjs`
...
- **Expected:** Returns 200 with ok:true, agentToken, cccUrl, deployKey, repoUrl
...
- **Expected:** Our token `<YOUR_CCC_TOKEN>` (listed as the agent/admin token) should work for admin-only endpoints
- **Actual:** `CCC_ADMIN_TOKEN` on the live server defaults to `process.env.CCC_ADMIN_TOKEN || process.env.CCC_AUTH_TOKENS?.split(',')[0]`. If the live server's `CCC_AUTH_TOKENS` has a different first token than ours, our token will fail admin checks with 401.
