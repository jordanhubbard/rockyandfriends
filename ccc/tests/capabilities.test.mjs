 ...
 * Run: node --test.ccc/tests/capabilities.test.mjs
 */
...
const TEST_QUEUE = join(tmpdir(), `ccc-caps-queue-${Date.now()}.json`);
const TEST_AGENTS = join(tmpdir(), `ccc-caps-agents-${Date.now()}.json`);
const TEST_CAPS   = join(tmpdir(), `ccc-caps-caps-${Date.now()}.json`);
...
process.env.BRAIN_STATE_PATH  = join(tmpdir(), `ccc-caps-brain-${Date.now()}.json`);
