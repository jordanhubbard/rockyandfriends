 ...
 *
 * Run: node --test.ccc/tests/api/exec.test.mjs
 */
...
const TEST_QUEUE  = join(tmpdir(), `ccc-exec-queue-${Date.now()}.json`);
const TEST_AGENTS = join(tmpdir(), `ccc-exec-agents-${Date.now()}.json`);
const TEST_CAPS   = join(tmpdir(), `ccc-exec-caps-${Date.now()}.json`);
const TEST_EXEC_LOG = join(tmpdir(), `ccc-exec-log-${Date.now()}.jsonl`);
...
process.env.BRAIN_STATE_PATH  = join(tmpdir(), `ccc-exec-brain-${Date.now()}.json`);
