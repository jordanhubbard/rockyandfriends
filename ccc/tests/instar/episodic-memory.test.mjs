/**
 * Tests for.ccc/memory/episodic.mjs (instar item wq-INSTAR-*-05)
 *
...
before(async () => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ccc-episodic-test-'));
...
describe('EpisodicMemory', () => {
  ...
  test('saveDigest and getDigest round-trip', async () => {
    ...
    assert.deepEqual(retrieved.themes, ['testing', 'ccc']);
  });
