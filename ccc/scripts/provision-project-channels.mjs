...
const CCC_API    = process.env.CCC_API    || 'http://localhost:8789';
const CCC_TOKEN  = process.env.CCC_TOKEN  || '';
const CCC_PUBLIC = process.env.CCC_PUBLIC || 'http://localhost:8788';
...
function cccProjectUrl(repo) {
  ...
  return `${CCC_PUBLIC}/projects/${encodeURIComponent(repo.full_name)}`;
}
...
function buildTopic(repo) {
  ...
  parts.push(`CCC: ${cccProjectUrl(repo)}`);
...
function buildWelcomeMessage(repo, channelName, workspace) {
  ...
  lines.push(`*CCC dashboard:* ${cccProjectUrl(repo)}`);
