 ...
 *   CCC_AUTH_TOKEN       default: ccc-agent-rocky-20maaghccmbmnby63so
...
const CCC_AUTH_TOKEN      = process.env.CCC_AUTH_TOKEN      || 'ccc-agent-natasha-eeynvasslp8mna9bipx';
const AGENT_NAME          = process.env.AGENT_NAME          || 'natasha';
const STATUS_FILE         = resolve(process.env.STATUS_FILE || '/home/jkh/.ccc/ollama-status.json');
...
async function reportToCCC(model, status) {
  const gpuTelemetry = getGpuTelemetry();
  const ram = getSystemRam();
  ...
  if (ram) {
    payload.ram = ram;
    // Flat top-level fields for ccc-server telemetry merge
  ...
  try {
    const res = await fetch(`${CCC_API}/api/heartbeat/${AGENT_NAME}`, {
      method: 'POST',
