#!/usr/bin/env node
/**
 *.ccc/scripts/clawchat-ingest-watcher.mjs — ClawChat SSE → Milvus ccc_messages
 *
 ...
 * and upserts into Milvus ccc_messages collection.
 *
 ...
 *   MILVUS_COLLECTION   default: ccc_messages
...
const COLLECTION     = process.env.MILVUS_COLLECTION   || 'ccc_messages';
...
// Upsert a message into Milvus ccc_messages
  ...
  while (true) {
    ...
    if (connected) {
      ...
      if (wasOffline) {
        ...
        // Log reconnect to Milvus ccc_messages if possible
        try {
          const client = await getMilvus();
