 ...
 * with the CCC API. The script reads from.ccc/agents/<name>.capabilities.json
 * and POSTs it to POST /api/agents/:name.
 *
 * Usage:
 *   AGENT_NAME=rocky CCC_API=http://localhost:8789 CCC_AUTH_TOKEN=xxx \
 *     node.ccc/scripts/publish-capabilities.mjs
 *
 *   # Or pass the capabilities file directly:
 *   node.ccc/scripts/publish-capabilities.mjs /path/to/rocky.capabilities.json
 *
 * Environment:
 *   AGENT_NAME        — agent name (overrides "name" field in caps file)
 *   CCC_API           — CCC base URL (default: http://localhost:8789)
...
const AGENT_NAME  = process.env.AGENT_NAME || process.env.CCC_AGENT_NAME;
const CCC_API     = process.env.CCC_API || process.env.CCC_PUBLIC_URL || 'http://localhost:8789';
...
const url = `${CCC_API}/api/agents/${encodeURIComponent(name)}`;
const payload = {
  capabilities,
...
try {
  ...
  if (res.ok) {
    console.log(`[publish-capabilities] ✓ ${name} capabilities published to ${CCC_API}`);
    if (data.token) {
  ...
  console.error(`[publish-capabilities] Cannot reach CCC at ${CCC_API}: ${err.message}`);
  process.exit(1);
}
