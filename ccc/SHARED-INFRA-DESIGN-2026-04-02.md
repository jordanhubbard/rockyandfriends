...
Every agent reads/writes:
  ...
  │    ccc_memory      ← all agent memories (scoped by agent)  │
  │    ccc_messages    ← cross-agent conversations              │
  │    ccc_lessons     ← fleet-wide lessons learned            │
  │    ccc_queue       ← work item semantic dedup               │
  ├─────────────────────────────────────────────────────────────┤
...
Milvus collections exist: `ccc_memory`, `ccc_lessons`, `ccc_queue`, `ccc_messages`, `ccc_memory_sparky`.
...
3. CCC embeds via tokenhub → upserts to Milvus `ccc_memory`
...
CCC embeds content via tokenhub (text-embedding-3-large)
  ↓
Upserts to Milvus ccc_memory (scope=private for daily notes, scope=fleet for lessons)
  ↓
...
```
Returns top-10 semantically similar memories from `ccc_memory` where `agent=natasha`.
...
- Upserts to `ccc_memory` (content-addressed ID, scope field)
...
- Rocky runs nightly: reads all agent daily notes from S3, extracts significant entries, upserts to fleet `ccc_memory` with `scope=fleet`, prunes stale local copies
...
| "I forgot what we discussed last week" | Vector recall from `ccc_memory` surfaces it from any agent's notes |
