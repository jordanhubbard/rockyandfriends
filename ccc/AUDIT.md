...
- *.ccc/wasm-dashboard/:** A separate, older Rust/Leptos dashboard attempt (19 files, Cargo.toml, Trunk.toml). The current dashboard is in .ccc/dashboard/`. `wasm-dashboard/` appears to be a dead predecessor — **candidate for deletion, verify with Rocky.**
- *.ccc/crush-server/:** 630 files (mostly node_modules). This is the "crush" coding agent integration that was disabled in `app.rs` comments (`// coding_agent::CodingAgent, // disabled — crush-server not deployed`). May be entirely superseded by the ACP/Codex delegation pattern. **Candidate for deletion, verify with Rocky.**
...
- **ClawBus duplication:** `squirrelbus/` at repo root has `receive-server.mjs`, `context-handoff.mjs`, `SPEC.md`. .ccc/api/index.mjs` has its own in-process bus at lines 173–238. These are two different implementations. The standalone `squirrelbus/` appears to be the original design; the in-process version is what's actually running. **Needs reconciliation.**
...
```
CCC_PORT                  Server port (default 8789)
...
3. Delete .ccc/wasm-dashboard/` (confirm with Rocky first)
4. Delete .ccc/heartbeat-local/` (confirm with Rocky first)
5. Confirm and delete .ccc/crush-server/` if disabled (confirm with Rocky — 630 files)
