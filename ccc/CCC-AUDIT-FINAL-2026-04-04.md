...
**The Rust migration is 85% complete.** The Rust `ccc-server` binary is already running in production on do-host1 as `ccc-api.service`. No Node.js process is running. All 24 route modules have been ported to Axum/Rust (5,930 lines of native Rust). The remaining work is:
...
4. **Config consolidation** (replace env vars with `ccc.json`)
...
|------|--------|---------|----------|--------|
| 8789 | `ccc-server` | CCC API | **Rust** ✅ | Running natively |
...
| File | Lines | Status |
|------|-------|--------|
| `api/index.mjs` | 2,121 | **DEAD** — replaced by ccc-server |
...
```
ccc/api/                    # Entire Node.js API (replaced by ccc-server)
ccc/brain/                  # Replaced by brain.rs
ccc/vector/                 # Replaced by memory.rs
ccc/lessons/                # Replaced by lessons.rs
ccc/issues/                 # Replaced by issues.rs
ccc/scout/                  # Functionality absorbed into queue/agents
ccc/llm/                    # Replaced by models.rs + providers.rs
ccc/exec/agent-listener.mjs # Replaced by exec.rs (keep index.mjs if used for Sweden fleet)
ccc/crush-server/           # Dead, disabled in code
ccc/guardrails/             # Dead
ccc/trust/                  # Dead duplicate
ccc/decision-journal/       # Dead
ccc/capabilities/           # Dead
ccc/heartbeat-local/        # Dead
ccc/wasm-dashboard/         # Dead predecessor to dashboard/
ccc/ideation/               # Dead
ccc/package.json            # No more Node deps
ccc/package-lock.json       # No more Node deps
```

**Keep:**
- .ccc/data/` — runtime data (JSON stores, being migrated to SQLite)
- .ccc/deploy/` — systemd units
- .ccc/docs/` — documentation
- .ccc/scripts/` — operational scripts (some still Node.js, but they're utilities, not core services)
- .ccc/dashboard/` — the Rust workspace (this IS the codebase now)
- .ccc/tests/` — integration tests
- .ccc/hooks/` — git hooks

#### 2. Verify `exec/agent-listener.mjs` status for Sweden fleet
The Sweden containers still run `agent-listener.mjs` — it's a ClawBus SSE client that executes remote commands. This is NOT the same as .ccc/exec/agent-listener.mjs` (the CCC route). Check:
...
- Use existing auth key from CCC secrets (`qdrant_fleet_key`)

**Effort:** 2-4 hours (REST API is similar shape, Qdrant has cleaner API)

#### 5. Config consolidation: env vars → `ccc.json`
`state.rs` still reads env vars (`CCC_PORT`, `AUTH_TOKEN`, etc.). Should migrate to:
- Single `ccc.json` config file (schema defined in RUST_MIGRATION_SPEC.md)
...
#### 8. Process supervisor (ccc manages service lifecycles)
Currently: tokenhub, clawchat-server, dashboard-server, SSH tunnels all managed by separate systemd units. Target: ccc-server spawns and health-checks these as child processes.
...
| Config consolidation | → P1 item #5 above (env vars → ccc.json) |
...
| Install story | → P1 item #5 (single binary + ccc.json) |
...
- [x] `ccc-server` Rust binary runs in production (no Node.js API process)
...
- [ ] Zero `.mjs` files remain in .ccc/` (delete dead code — P0)
- [ ] .ccc/package.json` deleted (P0)
- [ ] `ccc.json` replaces env vars (P1)
...
|----------|--------|------|--------|
| .ccc/AUDIT.md` | Natasha | 2026-04-01 | Superseded by this report |
| .ccc/SOA-AUDIT-2026-04-02.md` | Rocky | 2026-04-02 | Superseded by this report |
| .ccc/REFACTOR_SCOPE.md` | Rocky | 2026-04-01 | Superseded by this report |
| .ccc/RUST_MIGRATION_SPEC.md` | Bullwinkle | 2026-04-02 | Superseded by this report |
| .ccc/SHARED-INFRA-DESIGN-2026-04-02.md` | Rocky | 2026-04-02 | Supplementary (infra layout) |
