...
# If no path given, looks for ~/.ccc/sbom.json or.ccc/sbom/<AGENT_NAME>.json
...
# ── Find SBOM ──────────────────────────────────────────────────────────────
if [ -z "$SBOM_PATH" ]; then
  if [ -f "$HOME/.ccc/sbom.json" ]; then
    SBOM_PATH="$HOME/.ccc/sbom.json"
  elif [ -f "$(dirname "$0")/${AGENT_NAME}.json" ]; then
    SBOM_PATH="$(dirname "$0")/${AGENT_NAME}.json"
  else
    echo "ERROR: No SBOM found. Pass path as argument or place at ~/.ccc/sbom.json"
    exit 1
  fi
fi
...
if [ ${#ENV_MISSING[@]} -gt 0 ]; then
  ...
  echo "   Add them to ~/.ccc/.env and source it."
fi
