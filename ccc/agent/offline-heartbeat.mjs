//.ccc/agent/offline-heartbeat.mjs
...
async function postHeartbeat(cccUrl, agentName, host, status) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), HEARTBEAT_TIMEOUT_MS);
  try {
    const res = await fetch(`${cccUrl}/api/heartbeat/${encodeURIComponent(agentName)}`, {
      method: 'POST',
...
async function replayBuffered(db, cccUrl) {
  const pending = db.prepare('SELECT * FROM heartbeats WHERE replayed = 0 ORDER BY id ASC').all();
  for (const row of pending) {
    const ok = await postHeartbeat(cccUrl, row.agent_name, row.host, row.status);
    if (ok) {
...
export async function sendHeartbeat(cccUrl, agentName, host, status) {
  const ts = new Date().toISOString();
  const online = await postHeartbeat(cccUrl, agentName, host, status);
  const db = getDb();
  try {
    if (online) {
      await replayBuffered(db, cccUrl);
    } else {
