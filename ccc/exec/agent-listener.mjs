/**
 *.ccc/exec/agent-listener.mjs — ClawBus subscriber + remote code executor
 *
 * Listens on ClawBus for `ccc.exec` messages, verifies HMAC signatures,
 ...
 * - ALWAYS log every execution attempt to ~/.ccc/logs/remote-exec.jsonl
...
const LOG_DIR  = join(homedir(), '.ccc', 'logs');
...
async function subscribe() {
  // Support both legacy /bus/stream (old Node.js API) and /api/bus/stream (Rust ccc-server)
...
console.log(`[exec-listener] Starting agent=${AGENT_NAME} ccc=${CCC_URL} bus=${SQUIRRELBUS_URL}`);
subscribe().catch(err => {
