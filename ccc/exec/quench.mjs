/**
 *.ccc/exec/quench.mjs — Agent quench/pause mechanism via ClawBus
 *
 * Agents call checkQuench() between work units. If a quench signal has been
 * received (via ccc.quench ClawBus message), checkQuench() blocks for the
 * remaining pause duration before returning. No work is interrupted mid-task.
 *
 * ClawBus message schema (type: "ccc.quench"):
 *   {
 *     "type":             "ccc.quench",
 *     "from":             "<sender-agent>",
 *     "body": {
...
/**
 * Handle an ccc.quench SSE message from ClawBus.
 * Call this from agent-listener.mjs when message.type === 'ccc.quench'.
 */
