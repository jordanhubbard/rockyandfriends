...
```bash
git clone git@github.com:<your-org>/rockyandfriends.git ~/.ccc/workspace
```
...
- Creates `~/.ccc/` directory structure
- Copies `.env.template` → `~/.ccc/.env` (first run only)
...
# Or as a systemd service (see.ccc/deploy/systemd/agent-listener.service)
```
...
See [.ccc/docs/remote-exec.md`](docs/remote-exec.md) for full details, security model, and shell allowlist configuration.
