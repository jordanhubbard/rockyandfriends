/**
 *.ccc/vector/ingest.mjs — Ingest helpers for Milvus RAG pipeline
 *
 ...
 *                          memory files go to ccc_memory_sparky collection
 *   EMBED_BACKEND=remote → uses NVIDIA NIM text-embedding-3-large (3072-dim) [default]
 *                          memory files go to ccc_memory collection
 */
...
const MEMORY_COLLECTION = EMBED_BACKEND === 'local' ? 'ccc_memory_sparky' : 'ccc_memory';
 ...
 * Uses ccc_memory_sparky (768-dim local) when EMBED_BACKEND=local,
 * or ccc_memory (3072-dim remote) otherwise.
 */
...
/**
 * Ingest a queue item into ccc_queue collection.
 */
export async function ingestQueueItem(item) {
  try {
    ...
    await vectorUpsert('ccc_queue', id, text, {
...
/**
 * Ingest a lesson into ccc_lessons collection.
 */
export async function ingestLesson(lesson) {
  try {
    ...
    await vectorUpsert('ccc_lessons', id, text, {
 ...
 * Routes to ccc_memory_sparky (768-dim, GPU) when EMBED_BACKEND=local,
 * or ccc_memory (3072-dim, Azure) otherwise.
...
export async function ingestMessage(msg) {
  try {
    ...
    const id = createHash('md5').update(`clawchat:${msg.id}:${msg.ts}`).digest('hex');
    await vectorUpsert(MEMORY_COLLECTION, id, text.slice(0, 1000), {
      ...
      source:     `clawchat:${msg.id || 'unknown'}`,
...
export async function ingestMessages(msgs) {
  ...
  try {
    ...
    const items = msgs
      ...
      .map(msg => ({
        id:   createHash('md5').update(`clawchat:${msg.id}:${msg.ts}`).digest('hex'),
