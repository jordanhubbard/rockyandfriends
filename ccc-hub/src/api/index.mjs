/**
 *.ccc/api — Claw Command Center REST API
 *
 ...
 *
 * Port: CCC_PORT env var (default 8789)
...
// ── Config ─────────────────────────────────────────────────────────────────
const PORT            = parseInt(process.env.CCC_PORT || '8789', 10);
...
const CCC_PUBLIC_URL  = process.env.CCC_PUBLIC_URL || 'http://localhost:8789';
const AUTH_TOKENS  = new Set((process.env.CCC_AUTH_TOKENS || '').split(',').map(t => t.trim()).filter(Boolean));
const CCC_ADMIN_TOKEN = process.env.CCC_ADMIN_TOKEN || process.env.CCC_AUTH_TOKENS?.split(',')[0];
...
const SERVICES_CATALOG = [
  ...
  { id: 'clawchat',     name: 'ClawChat',       url: 'http://146.190.134.110:8790/',           desc: 'Self-hosted team chat (Slack replacement)', host: 'do-host1' },
  ...
  { id: 'agentfs',          name: 'ClawFS',            url: 'http://100.87.229.125:8791',             desc: 'Content-addressed WASM module store',      host: 'sparky'   },
...
async function indexPendingQueueItems() {
  try {
    ...
    console.log(`[dedup-indexer] Indexing ${active.length} active queue items into ccc_queue_dedup`);
    let indexed = 0;
    for (const item of active) {
      try {
        ...
        await fetch(`${MILVUS_URL}/v2/vectordb/entities/upsert`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            collectionName: 'ccc_queue_dedup',
...
const BOOTSTRAP_TOKENS_PATH = process.env.BOOTSTRAP_TOKENS_PATH || '/home/jkh/.openclaw/workspace.ccc/data/bootstrap-tokens.json';
...
function projectUrl(fullName) {
  return `${CCC_PUBLIC_URL}/api/projects/${encodeURIComponent(fullName)}`;
}
...
function buildProjectFromRepo(repo) {
  return {
    ...
    ccc_url:       projectUrl(repo.full_name),
...
function isAdminAuthed(req) {
  if (!CCC_ADMIN_TOKEN) return true; // no admin token configured = open (dev mode)
  const auth = req.headers['authorization'] || '';
  const token = auth.replace(/^Bearer\s+/i, '').trim();
  return token === CCC_ADMIN_TOKEN;
}
...
function dashboardHtml() {
  ...
  <style>
    ...
    .src-ccc{color:#58a6ff}.src-brain{color:#a371f7}.src-queue{color:#3fb950}.src-agent{color:#e3b341}
    /* Shared */
  ...
  <div class="topbar">
    <div class="topbar-logo">🐿️ <span>CCC</span> Claw Command Center</div>
...
function servicesHtml() {
  ...
  <div class="nav"><a href="/projects">Projects</a> &nbsp;·&nbsp; <a href="/">← CCC</a></div>
  <h1>Services</h1>
 ...
 * Topic:   "<description> | GitHub: <url> | Issues: <tracker> | CCC: <ccc_url>"
...
async function setSlackChannelMeta(channelId, project) {
  ...
  if (project.ccc_url)     parts.push(`CCC: ${project.ccc_url}`);
  ...
  await Promise.all([
    slackPost('conversations.setTopic', { channel: channelId, topic }).catch(e =>
      console.warn(`[ccc-api] setTopic ${channelId}: ${e.message}`)),
    slackPost('conversations.setPurpose', { channel: channelId, purpose }).catch(e =>
      console.warn(`[ccc-api] setPurpose ${channelId}: ${e.message}`)),
  ]);
}
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    // ── GET /api/presence — ClawChat presence map (no auth, 30s cache) ──
    ...
    // will call POST /api/tunnel/register once it has generated its key).
    {
      ...
      if (method === 'GET' && tpm) {
        ...
        const publicHost = CCC_PUBLIC_URL.replace(/^https?:\/\//, '').split(':')[0];
    ...
    if (method === 'GET' && path === '/api/drift') {
      ...
      try {
        ...
        const journal = new DecisionJournal({ agent: agentFilter || '_ccc', logPath, silent: true });
...
// ── UI: GET /playground — nanolang browser playground ────────────────
    ...
    if (method === 'GET' && path === '/api/bootstrap') {
      ...
      if (agents[entry.agent]?.token) {
        ...
        agentToken = `ccc-agent-${entry.agent}-${randomUUID().slice(0, 8)}`;
        agents[entry.agent] = {
          ...(agents[entry.agent] || {}),
          name: entry.agent,
      ...
      console.log(`[ccc-api] Bootstrap consumed for agent ${entry.agent} from ${req.socket?.remoteAddress}`);
      return json(res, 200, {
        ok: true,
        agent: entry.agent,
...
cat > ~/.ssh/ccc-deploy-key << 'DEPLOYKEY'
${kr.deployKey.trim()}
DEPLOYKEY
chmod 600 ~/.ssh/ccc-deploy-key
grep -q "ccc-deploy-key" ~/.ssh/config 2>/dev/null || cat >> ~/.ssh/config << 'SSHCFG'
Host github.com
...
`;
      ...
      const envLines = [`CCC_AGENT_TOKEN=${agentToken}`, `CCC_URL=${CCC_PUBLIC_URL}`, `AGENT_NAME=${entry.agent}`, `AGENT_ROLE=${roleHint}`];
      ...
      const rccHost = CCC_PUBLIC_URL.replace(/^https?:\/\//, '').split(':')[0];

      const script = `#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# CCC Agent Onboard — ${entry.agent} @ \$(date -u +%Y-%m-%dT%H:%M:%SZ)
# Generated by Claw Command Center (revamp v2)
# Usage: curl "${CCC_PUBLIC_URL}/api/onboard?token=<token>" | bash
# ═══════════════════════════════════════════════════════════════════════════════
set -euo pipefail

AGENT_NAME="${entry.agent}"
ROLE_HINT="${roleHint}"
CCC_URL="${CCC_PUBLIC_URL}"
CCC_HOST="${rccHost}"
REPO_URL="${repoUrl}"
...
stdout_logfile=\$HOME/.ccc/logs/openclaw-gateway.log
stderr_logfile=\$HOME/.ccc/logs/openclaw-gateway.log
OCEOF
...
REG_RESP=\$(curl -sf -X POST "\$CCC_URL/api/agents/register" \\
  ...
  }" 2>/dev/null) && echo "  ✅ Registered with CCC" || echo "  ⚠️  Registration returned error (may already exist — continuing)"
...
if [ -n "\$NEW_TOKEN" ] && [ "\$NEW_TOKEN" != "\$CCC_AGENT_TOKEN" ]; then
  sed -i "s|^CCC_AGENT_TOKEN=.*|CCC_AGENT_TOKEN=\$NEW_TOKEN|" ~/.ccc/.env
  CCC_AGENT_TOKEN="\$NEW_TOKEN"
  echo "  → Updated token in ~/.ccc/.env"
fi
echo ""
`;
  ...
  if [ -n "\$SEEDER_HOSTS" ]; then
    for SEEDER_HOST in \$SEEDER_HOSTS; do
      ...
      if rsync -av --progress --partial \\
          -e "ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -i ~/.ssh/ccc-worker-key" \\
          "ccc-worker@\${SEEDER_HOST}:\${VLLM_MODEL_DIR}/" \\
...
nohup python3 -m http.server \$PORT --directory "\$(dirname \$MODEL_DIR)" > "\$HOME/.ccc/logs/model-seeder.log" 2>&1 &
echo \$! > "\$PID_FILE"
...
TUNNEL_KEY="\$HOME/.ssh/ccc-tunnel-key"
...
if [ "\$KEY_WRITTEN" = "false" ] || [ "\$KEY_WRITTEN" = "unknown" ]; then
  ...
  echo "     Admin must manually add this key for tunnel user on CCC host:"
  echo "     \$TUNNEL_PUBKEY"
...
TUNNEL_CMD="ssh -N -R \${TUNNEL_PORT}:localhost:8080 -i \$HOME/.ssh/ccc-tunnel-key -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -o ExitOnForwardFailure=yes \${TUNNEL_USER_NAME}@\${CCC_HOST}"

if [ "\$HAS_SYSTEMD" = "true" ]; then
  mkdir -p "\$HOME/.config/systemd/user"
  cat > "\$HOME/.config/systemd/user/ccc-vllm-tunnel.service" << TUNNELEOF
[Unit]
Description=CCC vLLM Reverse SSH Tunnel for \${AGENT_NAME}
After=network-online.target

[Service]
Type=simple
ExecStart=/usr/bin/ssh -N -R \${TUNNEL_PORT}:localhost:8080 -i \$HOME/.ssh/ccc-tunnel-key -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -o ExitOnForwardFailure=yes \${TUNNEL_USER_NAME}@\${CCC_HOST}
Restart=always
RestartSec=10
...
TUNNELEOF
  systemctl --user daemon-reload && systemctl --user enable --now ccc-vllm-tunnel.service && \\
  ...
  TUNNEL_CONF_CONTENT="[program:ccc-vllm-tunnel]
command=\${TUNNEL_CMD}
autostart=true
autorestart=true
startsecs=5
startretries=999
stdout_logfile=\$HOME/.ccc/logs/tunnel.log
stderr_logfile=\$HOME/.ccc/logs/tunnel.log"
  # Write to user dir always (for reference)
  echo "\$TUNNEL_CONF_CONTENT" > "\$USER_CONF_DIR/ccc-vllm-tunnel.conf"
  # Try system dir (requires root/sudo)
  if sudo tee "\$SYS_CONF_DIR/ccc-vllm-tunnel.conf" > /dev/null 2>&1 <<< "\$TUNNEL_CONF_CONTENT"; then
    echo "  ✅ Conf written to system supervisord dir"
    supervisorctl reread 2>/dev/null && supervisorctl update 2>/dev/null && \\
      supervisorctl start ccc-vllm-tunnel && \\
    ...
    echo "     sudo tee \$SYS_CONF_DIR/ccc-vllm-tunnel.conf <<'EOF'"
    echo "\$TUNNEL_CONF_CONTENT"
    echo "EOF"
    echo "     sudo supervisorctl reread && sudo supervisorctl update && sudo supervisorctl start ccc-vllm-tunnel"
    echo "  → Falling back to nohup restart-loop..."
  fi"

else
  # nohup restart-loop fallback (containers without supervisord)
  TUNNEL_PID="\$HOME/.ccc/tunnel.pid"
  mkdir -p "\$HOME/.ccc/logs"
  # Write tunnel cmd to a small script so nohup doesn't choke on special chars
  TUNNEL_SCRIPT="\$HOME/.ccc/start-tunnel.sh"
...
TSCRIPTEOF
  ...
  nohup bash "\$TUNNEL_SCRIPT" > "\$HOME/.ccc/logs/tunnel.log" 2>&1 &
  echo \$! > "\$TUNNEL_PID"
  echo "  ✅ Tunnel started via nohup restart-loop (pid \$(cat \$TUNNEL_PID))"
  # Wire into .bashrc for container restarts
  grep -q "ccc-vllm-tunnel autostart" "\$HOME/.bashrc" 2>/dev/null || cat >> "\$HOME/.bashrc" << 'RCEOF'
# ccc-vllm-tunnel autostart (added by CCC onboard)
if [ -z "$(pgrep -f 'ccc-tunnel-key')" ]; then
  nohup bash -c "while true; do \${TUNNEL_CMD}; sleep 10; done" > \$HOME/.ccc/logs/tunnel.log 2>&1 &
fi
RCEOF
fi
...
echo "  CCC has been informed via registration capabilities."
echo ""
fi
...
stdout_logfile=\$HOME/.ccc/logs/vllm.log
stderr_logfile=\$HOME/.ccc/logs/vllm.log"
  ...
  mkdir -p "\$HOME/.ccc/logs"
  nohup bash -c "\${VLLM_START_CMD}" > "\$HOME/.ccc/logs/vllm.log" 2>&1 &
  VLLM_PID=\$!
  echo \$VLLM_PID > "\$HOME/.ccc/vllm.pid"
  echo "  ✅ vLLM started via nohup (pid \$VLLM_PID)"
  # Wire into .bashrc for container restarts
  grep -q "vllm-worker autostart" "\$HOME/.bashrc" 2>/dev/null || cat >> "\$HOME/.bashrc" << 'VLLMRCEOF'
# vllm-worker autostart (added by CCC onboard)
...
if [ "\$AGENT_ROLE" = "vllm-worker" ]; then
  ...
  SYNC_SCRIPT="\$HOME/.ccc/scripts/memory-sync.sh"
  mkdir -p "\$HOME/.ccc/scripts"
...
SYNCEOF
  ...
  if [ -f "\$SUPCONF_OC" ]; then
    ...
    sed -i 's|^command=.*openclaw.*|command=bash -c "\\$HOME/.ccc/scripts/memory-sync.sh pull \\&\\& openclaw gateway start --foreground; \\$HOME/.ccc/scripts/memory-sync.sh push"|' "\$SUPCONF_OC" 2>/dev/null && \\
      supervisorctl reread 2>/dev/null && supervisorctl update 2>/dev/null && \\
  ...
  if command -v mc &>/dev/null; then
    "\$HOME/.ccc/scripts/memory-sync.sh" pull 2>/dev/null && echo "  ✅ Initial memory pull complete" || echo "  ℹ️  No prior memory found in MinIO (first run)"
  fi
  echo ""
fi
...
if [ "\$AGENT_ROLE" = "vllm-worker" ]; then
  ...
  if [ "\$HAS_TAILSCALE" = "false" ]; then
    ...
    if [ "\$TUNNEL_UP" = "True" ]; then
      ...
      echo "     Check: systemctl --user status ccc-vllm-tunnel"
    fi
  else
...
`;
      ...
      entry.lastUsedAt = new Date().toISOString();
      saveBootstrapTokens();
      console.log(`[ccc-api] Onboard script generated for ${entry.agent} (role hint: ${roleHint}) use ${entry.useCount}/${entry.maxUses||3} from ${req.socket?.remoteAddress}`);
...
loadPackages();
        ...
        try {
          const listing = await new Promise((resolve, reject) => {
            _https.get({
              ...
              headers: { 'User-Agent': 'ccc-api/1.0', 'Accept': 'application/vnd.github.v3+json' },
            }, (r) => {
              let buf = '';
              r.on('data', d => buf += d);
          ...
          for (const dir of dirs) {
            try {
              const manifest = await new Promise((resolve, reject) => {
                ...
                _https.get({ hostname: u.hostname, path: u.pathname, headers: { 'User-Agent': 'ccc-api/1.0' } }, (r) => {
                  let buf = '';
                  r.on('data', d => buf += d);
          ...
          console.warn('[ccc-api] /api/pkg fetch error:', err.message);
        }
      }
    ...
    if (method === 'POST' && path === '/api/queue') {
      ...
      if (!isIdeaOrSkip) {
        ...
        if (exactDup) {
          console.log(`[ccc-api] Exact-title dedup: rejected "${body.title.slice(0,60)}" (matches active item ${exactDup.id})`);
          return json(res, 409, {
            ok: false,
            error: 'duplicate',
        ...
        try {
          ...
          if (embedResp.ok) {
            ...
            if (vector && vector.length === 768) {
              // Query ccc_queue_dedup for top-3 nearest neighbors
              ...
              if (searchResp.ok) {
                ...
                if (duplicate) {
                  console.log(`[ccc-api] Semantic dedup: rejected "${body.title.slice(0,50)}" (similarity=${duplicate.distance.toFixed(3)} ≥ ${DEDUP_THRESH} to "${duplicate.title?.slice(0,50)}" id=${duplicate.id})`);
                  return json(res, 409, {
                    ok: false,
                    error: 'duplicate',
          ...
          console.warn('[ccc-api] Semantic dedup gate error (non-fatal):', err.message);
        }
      }
      ...
      if (body.id && allIds.has(body.id)) {
        ...
        console.warn(`[ccc-api] ID collision on "${body.id}" — reassigned to "${itemId}"`);
      }
      ...
      if (typeof rawPriority === 'number') {
        ...
        console.warn(`[ccc-api] Numeric priority ${body.priority} coerced to "${rawPriority}" for item "${body.title?.slice(0,40)}"`);
      } else if (!VALID_PRIORITIES.has(rawPriority)) {
        console.warn(`[ccc-api] Unknown priority "${rawPriority}" for item "${body.title?.slice(0,40)}" — defaulting to "normal"`);
        rawPriority = 'normal';
      }
    ...
    if (method === 'POST' && itemFailMatch) {
      ...
      if (item.attempts >= (item.maxAttempts || 3)) {
        ...
        item.journal.push({ ts: now, author: 'ccc-api', type: 'dlq', text: `Moved to blocked — maxAttempts exceeded` });
      }
      await writeQueue(q);
      return json(res, 200, { ok: true, item });
    }
    ...
    if (method === 'POST' && path === '/api/agents/register') {
      ...
      const token = existingToken || `ccc-agent-${body.name}-${Math.random().toString(36).slice(2)}${Date.now().toString(36)}`;
      ...
      if (effectiveTailscaleIp && hasVllm) {
        ...
        console.log(`[ccc-api] ${body.name} registered with Tailscale vLLM at ${effectiveTailscaleIp}:${vllmPort}`);
      }
      ...
      if (agents[body.name].capabilities.vllm && process.env.TOKENHUB_URL && process.env.TOKENHUB_ADMIN_TOKEN) {
        ...
        if (providerUrl) {
          ...
          try {
            ...
            console.log(`[ccc-api] Registered ${body.name} as TokenHub provider ${providerId} → ${providerUrl}`);
          } catch (thErr) {
            console.warn(`[ccc-api] TokenHub registration failed for ${body.name}: ${thErr.message}`);
          }
        } else {
          console.log(`[ccc-api] ${body.name} has vLLM but no tunnel/tailscale assigned yet — skipping TokenHub registration`);
        }
      }
    ...
    if (method === 'POST' && agentNameMatch) {
      ...
      if (!agents[name]) {
        ...
        const token = `ccc-agent-${name}-${Math.random().toString(36).slice(2)}${Date.now().toString(36)}`;
        agents[name] = {
          name,
    ...
    if (method === 'POST' && completeMatch) {
      ...
      if (item.requestId) {
        try {
          ...
          console.error('[ccc-api] requestId linkage error:', e.message);
        }
      }
    ...
    if (method === 'POST' && projectChannelMatch) {
      ...
      await setSlackChannelMeta(body.channel_id, project).catch(e =>
        console.warn(`[ccc-api] setSlackChannelMeta ${body.channel_id}: ${e.message}`));
    ...
    if (method === 'POST' && path === '/api/bus/receive') {
      const body = await readBody(req);
      broadcastGeekEvent('bus_msg', body.from || 'unknown', body.to || 'all', 'ClawBus message');
      if (body.type === 'lesson') {
    ...
    if (method === 'POST' && path === '/api/slack/events') {
      ...
      if (event.type === 'app_mention' || (event.type === 'message' && event.channel_type === 'im' && !event.bot_id)) {
        ...
        try {
          ...
          console.error('[ccc-api] Slack event brain error:', e.message);
          await slackPost('chat.postMessage', {
            channel:   event.channel,
    ...
    // ── POST /api/slack/commands — Slack slash commands (/ccc ...) ─────────
    if (method === 'POST' && path === '/api/slack/commands') {
      ...
      if (cmdText === 'status' || cmdText === '') {
        ...
        await slackRespond(`*🐿️ CCC Agent Status*\n${statusText}`);
        return;
      }
    ...
    if (method === 'GET' && path === '/api/geek/topology') {
      ...
      const edges = [
        { from: 'rocky',      to: 'ccc-api',        type: 'persistent',  protocol: 'internal' },
      ...
      const agentsData = await readAgents().catch(() => ({}));
      // Dynamic: recent bus messages (last 50 lines of clawbus/bus.jsonl)
      let busMessages = [];
    ...
    // ── Secrets Store ─────────────────────────────────────────────────────
    // CCC is the sole secrets holder. Agents fetch what they need here.
    ...
    if (method === 'POST' && path.startsWith('/api/secrets/')) {
      ...
      console.log(`[ccc-api] Secret '${key}' written by admin`);
      return json(res, 200, { ok: true, key });
    }
    ...
    if (method === 'DELETE' && path.startsWith('/api/secrets/')) {
      ...
      delete store[key];
      await writeFile(secretsPath, JSON.stringify(store, null, 2), 'utf8');
      console.log(`[ccc-api] Secret '${key}' deleted by admin`);
    ...
    if (method === 'POST' && path === '/api/bootstrap/token') {
      ...
      const token = `ccc-bootstrap-${body.agent}-${randomUUID().slice(0, 8)}`;
      ...
      return json(res, 200, { ok: true, bootstrapToken: token, agent: body.agent, role, expiresAt,
        onboardCmd: `curl -fsSL "${CCC_PUBLIC_URL}/api/onboard?token=${token}" | bash` });
    }
    ...
    if (method === 'GET' && path === '/api/bootstrap') {
      ...
      return json(res, 200, {
        ...
        agentToken,
        cccUrl: CCC_PUBLIC_URL,
        secrets,  // full secrets bundle — agent should write to ~/.ccc/.env
      });
    }

    // ── POST /api/exec — broadcast exec payload via ClawBus (admin only) ──
    if (method === 'POST' && path === '/api/exec') {
      ...
      const BUS_URL   = process.env.SQUIRRELBUS_URL || `http://localhost:${process.env.CCC_PORT || 8789}`;
      ...
      try {
        const busResp = await fetch(`${BUS_URL}/bus/send`, {
          ...
          body: JSON.stringify({
            ...
            to:      body.target || 'all',
            type:    'ccc.exec',
            subject: `ccc.exec:${execId}`,
            body:    JSON.stringify(envelope),
          }),
        });
        busSent = busResp.ok;
      } catch (busErr) {
        console.warn('[ccc-api] ClawBus broadcast failed:', busErr.message);
      }
    ...
    if (method === 'POST' && path === '/api/llms') {
      ...
      try {
        ...
        console.log(`[ccc-api] LLM advertised: ${entry.agent} → ${entry.models.length} model(s) at ${entry.baseUrl}`);
        broadcastGeekEvent('llm_advertise', entry.agent, 'ccc', `${entry.agent} serving ${entry.models.map(m=>m.name).join(', ')}`);
        return json(res, 200, { ok: true, entry });
      } catch (err) {
    ...
    if (method === 'POST' && path === '/api/tunnel/request') {
      ...
      if (!assigned) {
        ...
        const comment = `ccc-tunnel-${label}`;
        ...
        try {
          ...
          keyWritten = true;
          console.log(`[ccc-api] Tunnel key added for ${agent} on port ${port}`);
        } catch (authErr) {
          keyWritten = false;
          console.warn(`[ccc-api] Could not write authorized_keys for ${agent}: ${authErr.message}`);
          console.warn(`[ccc-api] Admin must manually add: restrict,port-forwarding,permitopen="localhost:${port}" ${pubkeyTrimmed} ccc-tunnel-${label}`);
        }
        ...
        try {
          ...
          keyWritten = true;
          console.log(`[ccc-api] Tunnel key rotated for ${agent} on port ${assigned.port}`);
        } catch (authErr) {
          keyWritten = false;
          console.warn(`[ccc-api] Could not rotate authorized_keys for ${agent}: ${authErr.message}`);
        }
      }

      const publicHost = (CCC_PUBLIC_URL.replace(/^https?:\/\//, '').split(':')[0]) || '146.190.134.110';
      return json(res, 200, {
        ok:    true,
        port:  assigned.port,
        user:  TUNNEL_USER,
    ...
    if (method === 'POST' && path === '/api/tunnel/shell') {
      ...
      if (!assigned) {
        ...
        const comment = `ccc-shell-tunnel-${label}`;
        ...
        try {
          ...
          keyWritten = true;
          console.log(`[ccc-api] Shell tunnel key added for ${agent} on port ${port}`);
        } catch (authErr) {
          keyWritten = false;
          console.warn(`[ccc-api] Could not write shell tunnel authorized_keys for ${agent}: ${authErr.message}`);
        }
      } else if (assigned.pubkey !== pubkeyTrimmed) {
        assigned.pubkey = pubkeyTrimmed;
        assigned.updatedAt = new Date().toISOString();
        ...
        try {
          ...
          keyWritten = true;
          console.log(`[ccc-api] Shell tunnel key rotated for ${agent} on port ${assigned.port}`);
        } catch (authErr) {
          keyWritten = false;
          console.warn(`[ccc-api] Could not rotate shell tunnel authorized_keys for ${agent}: ${authErr.message}`);
        }
      }
    ...
    // ── POST /api/tunnel/verify — check if tunnel is active from CCC side ─
    ...
    // 5-minute cache. Polls ClawFS /health and returns synthesized slot state.
    if (method === 'GET' && path === '/api/agentos/slots') {
      ...
      // Probe ClawFS on sparky (content-addressed WASM store)
      ...
      // We derive slot health from the ClawFS /health response + stored metrics.
    ...
    if (method === 'GET' && path === '/api/mesh') {
      ...
      let spawnLog = [];
      try {
        const busPath = process.env.BUS_LOG_PATH || '/home/jkh/Src/CCC.ccc/data/bus.jsonl';
...
loadPackages();
    ...
    console.error('[ccc-api] Error:', err.message);
...
export function startServer(port = PORT) {
  ...
  server.listen(port, '0.0.0.0', () => {
    console.log(`[ccc-api] 🐿️ CCC API running on http://0.0.0.0:${port}`);
    console.log(`[ccc-api] Auth: ${AUTH_TOKENS.size > 0 ? `${AUTH_TOKENS.size} token(s) configured` : 'OPEN (no tokens set)'}`);
  });
  ...
  setImmediate(async () => {
    try {
      ...
      if (bareLines.length > 0) {
        console.warn(`[ccc-api] ⚠️  authorized_keys has ${bareLines.length} bare pubkey(s) without restrict/permitopen guards:`);
        bareLines.forEach(l => console.warn(`[ccc-api]    ${l.slice(0, 80)}...`));
        console.warn(`[ccc-api]    These should be prefixed with: restrict,port-forwarding,permitopen="localhost:<PORT>"`);
      }
...
async function reloadAgentTokens() {
  try {
    ...
    if (reloaded > 0) console.log(`[ccc-api] Reloaded ${reloaded} agent token(s) from agents.json`);
  } catch (e) {
    console.warn('[ccc-api] Could not reload agent tokens:', e.message);
  }
}
...
async function initLLMRegistry() {
  ...
  console.log('[ccc-api] LLM registry initialized');
}
    ...
    await withQueueLock(async () => {
      ...
      for (const item of (q.items || [])) {
        ...
        if ((now - new Date(item.claimedAt).getTime()) > threshold) {
          ...
          item.status = 'pending';
          if (!item.journal) item.journal = [];
          item.journal.push({ ts: new Date().toISOString(), author: 'ccc', type: 'stale-reset', text: `Auto-reset stale claim (was ${prev})` });
          reset++;
        }
      }
      if (reset > 0) {
        await writeQueue(q);
        console.log(`[ccc-api] Auto-expired ${reset} stale claim(s)`);
      }
    });
  } catch (e) {
    console.error('[ccc-api] Stale expiry error:', e.message);
  }
}, 5 * 60 * 1000).unref();
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  reloadAgentTokens()
    ...
    .then(() => {
      ...
      // Background: index existing pending queue items into ccc_queue_dedup (once at startup, best-effort)
