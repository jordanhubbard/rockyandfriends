/**
 *.ccc/scout/pump.mjs — The Work Pump
 *
 ...
 *
 * Runs as part of ccc-api or standalone.
 */
...
const CCC_API            = process.env.CCC_API_INTERNAL || 'http://localhost:8789';
const CCC_TOKEN          = process.env.CCC_AUTH_TOKENS?.split(',')[0] || '';
...
async function cccGet(path) {
  const r = await fetch(`${CCC_API}${path}`);
  return r.json();
}
...
async function runScan() {
  ...
  const { items = [], completed = [] } = await cccGet('/api/queue');
  ...
  for (const item of newItems) {
    try {
      const result = await cccPost('/api/queue', item);
...
async function checkIdle() {
  const { items = [] } = await cccGet('/api/queue');
  const agents = await cccGet('/api/agents');
...
async function verifyCompletions() {
  const { completed = [] } = await cccGet('/api/queue');
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  ...
  if (cmd === 'scan') {
    ...
    console.log('Usage: node.ccc/scout/pump.mjs scan|idle-check|register <repo>|list');
  }
}
