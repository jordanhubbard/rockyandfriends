/**
 *.ccc/lessons/index.mjs — Distributed Lessons Learned Ledger
 *
 ...
 * Local cache: ~/.ccc/lessons/<domain>.jsonl (fast lookup, no MinIO roundtrip)
...
// ── Config ─────────────────────────────────────────────────────────────────
const LESSONS_DIR   = process.env.LESSONS_DIR   || join(process.env.HOME || '/tmp', '.ccc/lessons');
...
const BUS_TOKEN     = process.env.BUS_TOKEN     || process.env.CCC_AUTH_TOKENS?.split(',')[0] || '';
 ...
 *
 * @param {object} lesson
 * @param {string} lesson.domain    - e.g. "express", "github", "ci", "ccc", "python"
