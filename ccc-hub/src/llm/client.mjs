/**
 *.ccc/llm/client.mjs — Peer LLM Client
 *
 ...
 *     cccUrl:   'http://do-host1:8789',
 *     cccToken: process.env.CCC_AGENT_TOKEN,
 *   });
 *
 *   // Chat
...
export class PeerLLMClient {
  /**
   * @param {object} opts
   * @param {string} opts.cccUrl      — CCC base URL, e.g. http://do-host1:8789
   * @param {string} opts.cccToken    — Agent bearer token for CCC
   * @param {number} [opts.timeoutMs] — Per-request timeout (default 60s)
   */
  constructor({ cccUrl, cccToken, timeoutMs = 60_000 } = {}) {
    this.cccUrl   = (cccUrl   || process.env.CCC_URL    || 'http://localhost:8789').replace(/\/$/, '');
    this.cccToken = cccToken  || process.env.CCC_AGENT_TOKEN || '';
  ...
  async _rccFetch(path, opts = {}) {
    const url = `${this.cccUrl}${path}`;
    const headers = {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${this.cccToken}`,
      ...opts.headers,
    };
