# CCC Hub — Docker Compose Deployment
...
# 2. Configure
cp .env.template .env
$EDITOR .env   # fill in CCC_PORT, CCC_AUTH_TOKENS, CCC_ADMIN_TOKEN at minimum
...
|-----------------|-------|---------------------------------------|
| `ccc-api`       | 8789  | Node.js REST API + work queue         |
| `ccc-dashboard` | 8788  | Rust/WASM dashboard (serves frontend) |
...
CCC_ADMIN_TOKEN=your-admin-token
```
...
| Volume           | Contents                                  |
|------------------|-------------------------------------------|
| `ccc-hub-data`   | Queue, agents, logs, brain state          |
| `ccc-minio-data` | MinIO objects (only when minio profile on)|

To back up data:

```bash
docker run --rm -v ccc-hub-data:/data -v $(pwd):/backup debian \
  tar czf /backup/ccc-data-$(date +%Y%m%d).tar.gz /data
```
...
```bash
# View logs
docker compose logs -f ccc-api

# Restart just the API
docker compose restart ccc-api
...
The `ccc-dashboard` service expects a pre-built Rust binary and WASM frontend.
Build on the host before deploying:

```bash
cd.ccc/dashboard
make release
cp target/release/dashboard-server ../ccc-hub/dashboard-server
cp -r dist/ ../ccc-hub/dist/
```
