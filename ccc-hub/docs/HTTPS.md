...
- CCC already running (see [DOCKER.md](DOCKER.md) or the manual install)
...
Edit `Caddyfile` in the ccc-hub directory and replace `ccc.yourdomain.com` with your domain:

```bash
sed -i 's/ccc.yourdomain.com/ccc.example.com/g' Caddyfile
```
...
```bash
curl https://ccc.example.com/health
# → {"status":"ok",...}
```
...
sudo sed -i 's/ccc.yourdomain.com/ccc.example.com/g' /etc/nginx/sites-available/ccc

sudo ln -s /etc/nginx/sites-available/ccc /etc/nginx/sites-enabled/
sudo nginx -t
...
```bash
sudo certbot --nginx -d ccc.example.com
```
...
The install wizard in `install-hub.sh` will prompt for TLS setup when your `CCC_PUBLIC_URL`
...
Set `CCC_PUBLIC_URL=https://ccc.example.com` in `.env` after TLS is configured.
...
# Optionally restrict direct access to CCC ports:
...
| Caddy: `certificate authority failed` | Check DNS has propagated (`dig +short ccc.example.com`) |
| nginx: 502 Bad Gateway | Confirm `ccc-api` is running on port 8789 |
