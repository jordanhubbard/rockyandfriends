use crate::db::{
    db_fleet_sync_claim, db_fleet_sync_complete, db_fleet_sync_fail, db_fleet_sync_keepalive,
};
...
async fn post_queue(
    ...
    {
        let description_str = body
            .get("description")
            .and_then(|s| s.as_str())
            .unwrap_or("");
        let assignee_str = body
            .get("assignee")
            .and_then(|s| s.as_str())
            .unwrap_or("all");
    ...
    let _ = state
        .bus_tx
        .send(serde_json::json!({"type": "work.available", "item_id": final_id}).to_string());
...
async fn delete_item(
    ...
    if let Some(pos) = item_pos {
        ...
        return (
            StatusCode::OK,
            Json(json!({"ok": true, "item": item, "cancelled": true})),
        )
            .into_response();
    }
    ...
    (
        StatusCode::NOT_FOUND,
        Json(json!({"error": "Item not found or already completed"})),
    )
        .into_response()
}
    ...
    match item_pos {
        ...
        Some(pos) => {
            ...
            if attempts >= max_attempts {
                ...
                let dlq_entry = json!({"ts": now, "author": "ccc-api", "type": "dlq", "text": "Moved to blocked — maxAttempts exceeded"});
                if let Some(j) = obj.get_mut("journal").and_then(|j| j.as_array_mut()) {
                    j.push(journal_entry);
                    j.push(dlq_entry);
                } else {
...
async fn stale_reset_item(
    ...
    match item_pos {
        ...
        Some(pos) => {
            ...
            let attempts = obj.get("attempts").and_then(|n| n.as_u64()).unwrap_or(0) + 1;
            obj.insert("attempts".into(), json!(attempts));
            let entry = json!({"ts": now, "author": "ccc-api", "type": "stale-reset", "text": format!("Manual stale reset (was {})", prev_agent)});
            if let Some(j) = obj.get_mut("journal").and_then(|j| j.as_array_mut()) {
                j.push(entry);
            } else {
