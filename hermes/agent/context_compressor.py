...
def _content_text_for_contains(content: Any) -> str:
    ...
    Keeps multimodal lists intact elsewhere — only used when we need to know
    whether a note has already been appended.
    """
    if content is None:
        return ""
...
def _append_text_to_content(content: Any, text: str, *, prepend: bool = False) -> Any:
    ...
    Handles both plain-string content and multimodal lists of blocks so
    direct string concatenation never corrupts vision-heavy messages.
    """
    if content is None:
        return text
    if isinstance(content, str):
