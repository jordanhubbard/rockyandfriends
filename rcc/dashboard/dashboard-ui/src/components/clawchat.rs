...
/// Fetch channel list from clawchat-server via /sc proxy.
...
pub fn ClawChat() -> impl IntoView {
    ...
    create_effect(move |_| {
        let shim = r#"
if (!window.__clawchat_voice) {
window.__clawchat_voice = (function() {
        ...
        navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(function(stream) {
            ...
            try {
                ...
                analyserInterval = setInterval(function() {
                    if (muted) { window.__clawchat_voice._speaking = false; return; }
                    analyser.getByteTimeDomainData(data);
                    var rms = 0;
                    for (var i=0; i<data.length; i++) rms += (data[i]-128)*(data[i]-128);
                    window.__clawchat_voice._speaking = Math.sqrt(rms/data.length) > 10;
                }, 150);
            } catch(e2) {}
    ...
    function leaveVoice() {
        ...
        window.__clawchat_voice._speaking = false;
    }
    function muteVoice() { muted=true; if(localStream)localStream.getAudioTracks().forEach(function(t){t.enabled=false;}); _speaking=false; window.__clawchat_voice._speaking=false; }
...
"#;
    ...
    // ── WebSocket connection to clawchat-server ───────────────────────────
    // WebSocket connects directly to clawchat-server (8793); dashboard-server
            ...
            if active_sess.is_some() {
                ...
                spawn_local(async move {
                    loop {
                        ...
                        let speaking = js_sys::eval("(window.__clawchat_voice && window.__clawchat_voice._speaking) ? true : false")
                                                        ...
                                                        .await
                                                    {
                                                        if let Ok(data) = resp.json::<serde_json::Value>().await {
                                                            if let Some(sid) = data["session_id"].as_str() {
                                                                ...
                                                                let js_call = format!(
                                                                    "window.__clawchat_voice && window.__clawchat_voice.joinVoice('{}', {})",
                                                                    sid,
                                        ...
                                        {if is_active {
                                            ...
                                            view! {
                                                <span
                                                    ...
                                                    on:click=move |ev| {
                                                        ev.stop_propagation();
                                                        let _ = js_sys::eval("window.__clawchat_voice && window.__clawchat_voice.leaveVoice()");
                                            ...
                                            <div class="sc-voice-controls">
                                                <button
                                                    ...
                                                    on:click=move |_| {
                                                        ...
                                                        if now_muted {
                                                            let _ = js_sys::eval("window.__clawchat_voice && window.__clawchat_voice.muteVoice()");
                                                        } else {
                                                            let _ = js_sys::eval("window.__clawchat_voice && window.__clawchat_voice.unmuteVoice()");
                                                        }
                                                    }
                                                >
                                                    {if muted_now { "🔇" } else { "🎤" }}
