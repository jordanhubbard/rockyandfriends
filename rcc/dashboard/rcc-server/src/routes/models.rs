...
async fn trigger_deploy(
    ...
    let script_path = std::env::var("DEPLOY_SCRIPT")
        .unwrap_or_else(|_| {
            ...
            let candidates = [
                "/home/jkh/Src/rockyandfriends/rcc/scripts/model-deploy.mjs",
                "./rcc/scripts/model-deploy.mjs",
                "./scripts/model-deploy.mjs",
            ];
            candidates.iter()
                .find(|p| std::path::Path::new(p).exists())
                .map(|s| s.to_string())
                .unwrap_or_else(|| "./rcc/scripts/model-deploy.mjs".to_string())
        });
    ...
    let env_token = state.secrets.read().await
        .get("RCC_AGENT_TOKEN").cloned()
        .map(|v| v.as_str().unwrap_or("").to_string()).unwrap_or_else(|| std::env::var("RCC_AGENT_TOKEN").unwrap_or_default());
    ...
    tokio::spawn(async move {
        ...
        cmd.args(&args)
            .env("RCC_AGENT_TOKEN", &env_token)
            .env("HF_TOKEN", &hf_token)
            .env("DEPLOY_ITEM_ID", "")
            .env("RCC_URL", std::env::var("RCC_URL").unwrap_or_else(|_| "http://localhost:8789".to_string()))
            .stdout(stdio)
